# -*- coding: utf-8 -*-
"""
Created on Sun Dec 25 23:25:19 2022
...
def get_size(bytes):
    """
    Returns the size in good format
    which is no. of bytes, KB ,MB , GB or TB 
    """
    for u in ['', 'K', 'M', 'G', 'T', 'P']:
        if bytes < 1024:
            return f"{bytes:.2f}{u}B"
        bytes /= 1024

'''
psutil.process_iter():
Return an iterator yielding a Process class instance for all running processes on the local machine.
 This should be preferred over psutil.pids() to iterate over processes as it’s safe
 from race condition
 
- In order to efficiently fetch more than one information about the process at the same time, 
    make sure to use oneshot()
    '''
...
def construct_process_df(processes):
...
if __name__ == "__main__":
    import argparse 
    import csv
    parser = argparse.ArgumentParser(description="Process Viewer & Monitor(Simulation to Task manager)")
    ...
    processes = get_processes_information()
    df = construct_process_df(processes)
    # df=processes
    print(f"Number of printable processes: {n}\n")
    ...
    while live_update:
        ...
        if n == 0:
            ...
            print(df.head(n).to_string())
        time.sleep(1)
    file =open('running processes.csv',mode='w',newline='')
    file.write(df.head(n).to_string())
    file.close()
